# Arena of Valor 專案 — Gemini 工作守則

> 此檔給 Gemini CLI 讀取；跨家共用規則來自全域 `~/.gemini/GEMINI.md`。本檔為專案層補充。
...
- 查歷史先 `grep -n "^### " TASK_HISTORY.md` 探錨點 → 精讀 offset 段落（≤200 行）
- 寫新 Phase 用 `cat >> TASK_HISTORY.md << 'EOF'`，不用編輯工具整檔覆寫
- 主公唯一觸發詞「這次」+「全讀」才放行（二次確認後才執行）
- 一次對話最多查 3 個 Phase（原子查詢守則）
