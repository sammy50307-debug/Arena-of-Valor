...
3. `docs/PHASE_102_PLAN.md`（P102 Missing Guard Backlog / Monitoring Review DRAFT）
4. `docs/KNOWN_ISSUE_GUARD_INDEX.md`（P101/P102 guard map）
5. `docs/RISK_REGISTRY.md` 的 R-023 / R-022 / R-017 / R-016（monitoring review；known issue index；content trust；backend monitoring）
6. `docs/PHASE_101_PLAN.md`（P101 Known Issue Guard Index CLOSED）
7. `scripts/check_known_issue_guard_index.py`（P101 advisory checker）
8. `docs/CONTENT_TRUST_KNOWN_ISSUES.md`（R-017 known issue human memory）
9. `data/runs/2026-05-29/run_manifest.json`（latest local production manifest evidence）
10. `data/reports/aov_report_2026-05-29.html`（latest local production report evidence）
...
| L3 | `docs/PHASE_102_PLAN.md` | P102 Missing Guard Backlog / Monitoring Review DRAFT |
| L3-p101 | `docs/PHASE_101_PLAN.md` | P101 Known Issue Guard Index CLOSED |
| L3-index | `docs/KNOWN_ISSUE_GUARD_INDEX.md` | P101/P102 known issue / guard / command / gap map |
...
| L4 | `docs/RISK_REGISTRY.md` 的 R-023 / R-022 / R-021 / R-020 / R-019 / R-018 / R-017 / R-016 | monitoring review；known issue guard index；root quarantine；artifact hygiene；project flywheel；RTK tooling；content trust monitoring；backend monitoring |

## Window Switch Timing

| 判斷 | 建議 |
|---|---|
| **現在能不能換視窗** | 可以。若本地 ahead 1 是 P102 plan draft commit，下一窗先等主公確認 push。 |
| **最舒服的換窗點** | P102 plan draft commit / push 完成，等待主公裁決 `核准 P102 plan freeze`。 |
| **如果現在立刻換** | 新視窗第一動作：讀本檔頂部 → `git status -sb` → `git log -1 --oneline` → 讀 `docs/PHASE_102_PLAN.md` 與 `docs/RISK_REGISTRY.md` 的 R-023。 |
| **不要在換窗後做的事** | 不要關閉 R-016/R-017；不要補所有 missing guards；不要改 checker；不要接 strict gate；不要清理 root/generated files。 |
...
- 不要把 P102 DRAFT 解讀成 R-016/R-017 可關閉；P102 plan 只做 review / ranking，runtime 仍需主公核准。
