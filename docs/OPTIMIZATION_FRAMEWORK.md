...
回應「63 維度認知負擔重」。依 META3 影響半徑分級：

| 半徑 | 動檔範圍 | 稽核強度 | 計畫書 |
|---|---|---|---|
| **微 Phase** | 1-2 檔 | 僅 S 級 4 層必填，A/B 全跳過（除非主公點名） | 對話即可，不凍結 |
| **標準 Phase** | 3-9 檔 | S + A 共 11 層必填，B 級條件式 | 凍結 PHASE_##_PLAN.md |
| **重大 Phase** | 10+ 檔 | 全 17 層 + 全治理層 + 全跨切面 | 凍結 + 主公親核 + 風險登記簿更新 |

**判斷時機**：草案完成時就要評估影響半徑，並明文寫進元資料表。

### Patch-2：B 級層觸發判斷 SOP

回應「B 級條件式判斷需主觀」。改為「機械式關鍵字觸發」：

| 層 | 觸發關鍵字（檔案/詞彙含以下 = 觸發） |
|---|---|
| **8. 效能層** | `for/while` 迴圈處理 ≥ 100 筆、`async`、`hook`、即時 UI、實時排序 |
| **9. UX/A11y 層** | `.html` / `.css` / `templates/` / 模板渲染 / 任何前端檔變更 |
| **11. 部署層** | `.github/workflows/` / `settings.json` / `.env` / Docker / CI 排程 |
| **12. 成本層** | `gemini` / `openai` / `anthropic` / `tavily` / API key / token / cost |
| **16. 隱私/合規層** | `scraper` / `dcard` / `bahamut` / `facebook` / `instagram` / 抓取使用者貼文 |
| **17. i18n 層** | `regional` / `TW`/`TH`/`VN` 三區同時動 / `locales/` |

**規則**：草案中只要動到的檔案路徑或新增邏輯出現上述關鍵字，對應 B 級層即「觸發」，必須填表。

### Patch-3：G5-1 vs X3 邊界釐清

回應「G5-1 與 X3 部分功能重疊」。明文劃界：

| 維度 | G5-1 規則退化警示 | X3 時間敏感性 |
|---|---|---|
| **觸發機制** | 規則「未被使用」90 天 → 警示 | 規則/決議「明文到期日」抵達 → 警示 |
| **語意差異** | 沒人用 = 可能死規則 | 過期 = 該重新審視 |
| **適用對象** | 永久性規則（律法、四層防線） | 時效性決議（Phase 計畫過期日、風險登記項） |
| **處置方式** | 主公裁示保留/退役 | 強制重新審視，可續期或關閉 |
| **舉例** | 三禁詞「壓縮/摘要/簡化」90 天無觸發 | Phase 65 計畫凍結後 3 個月過期重審 |

**白話**：G5-1 管「沒人用」，X3 管「過期了」。兩者可同時觸發但不重複。
...
- **v3.1** (2026-05-03)：依 Phase 64 回溯診斷補入 3 條 Patch（微 Phase 簡化、B 級觸發 SOP、G5-1/X3 邊界）
