...
- **緩解策略**：
  ...
  - 已凍結：P93 `Provider Abstraction / Disabled-by-default Free Provider Slots` 已 FROZEN；計畫明列 Groq / Cloudflare Workers AI / GitHub Models 只能作為 disabled-by-default 候選插槽，不得自動接 daily default，不新增 provider key / PAT / Cloudflare token / Groq key，不加 GitHub Actions `models: read`，且 runtime 必須另過主公核准、budget guard、secret/privacy、fake-provider tests、manual-only smoke gate。P93 plan-only 未新增 runtime code，R-016 仍 Open。
  - 中期：P86-P95 分段處理 model/schedule、report core contract、本地 deterministic analysis、quality tier、budget ledger、cache/dedupe、enrichment replay、provider abstraction、doctor/SLO 重分類；下一步是 P93 runtime 是否動工由主公另行核准，或先 push P93 FROZEN commit。
