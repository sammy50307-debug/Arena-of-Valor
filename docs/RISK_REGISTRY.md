...
- **緩解策略**：
  ...
  - 已凍結：P89 `Quality Tier / Promotion Gate` 計畫已建立，目標是新增 `quality_tier` 與 promotion gate matrix，讓 `production_local_only` 在 core contract / local baseline 通過時可發布；P89 plan-only 未改 runtime，R-016 仍 Open。
