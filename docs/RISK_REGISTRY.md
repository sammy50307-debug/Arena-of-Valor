...
- **緩解策略**：(a) **P112 已補 self-heal 保留 `pre_heal_error`**（持久化 generate 失敗原因供診斷，commit 進 manifest、不隨覆蓋遺失，2026-06-14 收官）；主根因 `main.py:728` 吞例外仍 known issue（刻意 graceful degradation，0 次發生不治本）。**🔑 PROMOTE 觸發（數據驅動，非日期）：若 manifest `self_heal=true` recurring（≥3 次/30 天，`grep -l '"self_heal": true' data/runs/*/run_manifest.json`）→ 升級治本 main.py:728 + 加 health checker（C′）；否則維持（self-heal backstop + pre_heal_error 診斷足夠）**。(b) 退化保真：manifest 標 `self_heal/is_backfill` 可辨識自癒產出 vs 正式產出；(c) 自動化邊界：`replay_run.py` docstring 契約 + postmortem 通則1/2 + G-ii sys.modules guard + 「不可逆仍問阿喜」鐵律；改 `should_promote`/`run_checks` 參數/heal 流程任一處須重審 L5 同源性（X3 過期日 2026-09-14）。
- **關聯**：B-027（凍結計畫字面修法可能與真實架構矛盾且測試抓不到）；**B-028（勿為 0 次失敗 speculative observability，改數據驅動 Phase 觸發）**；R-038（no-op candidate 進版控）；postmortem 4 通則；runbook SELF-HEAL 診斷節。
...
- **緩解策略**：
  - 短期：本條目以 X3 到期日 `2026-08-31` 錨定，靠 STR8「每 5-10 phase 復盤」掃到（**註：STR8 曾停擺 ~14 Phase，2026-06-14 P113 已補做並重啟，下次約 P117**）。
  - 中期：2026-08-31 前的跨-phase 復盤，人工回顧守則三個月內是否真正擋下亂派／幻覺（可對照守則「自稽收斂判準句」出現頻率與當下判斷）。
  - 長期：有效 → 保留或考慮升級為 checker；無效或過吵 → 降級為「建議」或移除，避免規則膨脹。
- **觸發升級／到期動作**：⚠️ **名實修正（P113，2026-06-14）**——原寫「到期由 G5-1 dead rule 偵測標記」**不成立**：G5-1（`scripts/rule-decay-check.sh`）只掃 `memory/*.md` 的 git mtime，**不讀 RISK_REGISTRY 的日期欄**，無法自動標記本條到期。故本條到期動作為**人工**：須於 2026-08-31 前的 STR8 復盤手動回顧；復盤判定無效卻保留 → 升為規則腐化 active issue，需明文降級或移除並記錄理由。（若要機器化，需在 governance_doctor 加 RISK 日期掃描——列低優先 future，非本期。）
...
- **2026-06-15**：P117 收官（對齊 Hermes Sentinel #1，跨專案飛輪盤點觸發）。交叉核對翻案——AOV 早有 `gov/preflight.py` 統一編排器（P103 從 skills-governance 快照，含防遞迴 GOV_PREFLIGHT_RUNNING），跨專案 audit 漏看。真缺口＝只註冊 4 個 check。本 Phase 把散落的 9 個 checker 註冊進 governance_config.yaml（ci profile 4：report_health/system_doctor/report_freshness/slo；full profile +5：known_issue_guard/artifact_hygiene/root_legacy/handoff_truth/no_fake_stats），daily_report.yml 接管 CI（4 個個別 step→一句 `python -m gov.preflight --profile ci`）。checker 散落無單一入口的治理債收斂。525 passed。偏離記錄：加 ci 到 preflight choices（1-token）、content_trust 延後（--date 無 today 預設）、credibility 排除（檔案參數 CLI + 已 inline）。CI 接管真驗證待下次 cron。
