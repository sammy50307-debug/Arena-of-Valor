...
- **緩解策略**：
  ...
  - 已凍結：P87 `Report Core Contract` 計畫已建立，採 shadow/advisory 路線先定義 `quality.core_contract`，讓 manifest / doctor / health 能判斷真實資料是否足以 production；P87 不直接改 quality tier / promotion gate，不關閉 R-016。
