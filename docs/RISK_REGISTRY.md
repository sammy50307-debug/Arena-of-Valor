...
- **緩解策略**：
  ...
  - 已凍結：P92 `Enrichment Replay / Local-only 補深讀` plan 已 FROZEN；採 artifact-backed enrichment queue + budget-aware replay 設計，raw queue 不進 repo，manifest 只寫 raw-free enrichment snapshot，`duplicate_url` / `duplicate_signature` local-only 預設 skipped / no-op。P92 runtime 尚未核准，未核准前不得修改 `main.py` / `analyzer/` / `scripts/` runtime。R-016 仍 Open。
  - 中期：P86-P95 分段處理 model/schedule、report core contract、本地 deterministic analysis、quality tier、budget ledger、cache/dedupe、enrichment replay、doctor/SLO 重分類；下一步是等待主公核准 P92 runtime，或進行只讀設計審查。
